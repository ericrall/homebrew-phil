...
**This repo now lives at `~/Developer/homebrew-phil`.** It was moved out of `~/Documents` on
...
| | |
|---|---|
| path | `~/Documents/homebrew-phil` |
| size | 296K |
| conflict copies present | **0** |
| uncommitted (tracked) | 1 |
| untracked | 0 |
| own `.venv` | no |
| remote | `github.com/ericrall/homebrew-phil` — **PUBLIC tap.** Also collides by name with `~/Developer/homebrew-phil`. |
